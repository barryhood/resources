class DxSlider {
  constructor(element, options = {}) {
    ...
    this._defaults = {
      multiThumb: true, // set to true to enable two thumbs
      classes: {
        thumb: 'DxSlider__thumb',
        thumbFirstMod: 'DxSlider__thumb--first',
        thumbSecondMod: 'DxSlider__thumb--second',
        railOverlay: 'DxSlider__rail-overlay',
        railOverlayPreMod: 'DxSlider__rail-overlay--pre',
        railOverlayMidMod: 'DxSlider__rail-overlay--mid',
        railOverlayPostMod: 'DxSlider__rail-overlay--post',
      },
    };
    this.controls = null;
  ...
  buildHTML() {
    const handleAttribs = 'role="slider" aria-valuemin="" aria-valuenow="" aria-valuemax="" tabindex="0"';
    const handleHtml = modifier => `<div class="DxSlider__thumb ${modifier}" ${handleAttribs}></div>`;
    const secondThumb = () => (this.options.multiThumb ? handleHtml('DxSlider__thumb--second') : '');

    const html = `
      <div class="DxSlider__rail">
        <div class="DxSlider__rail-overlay DxSlider__rail-overlay--pre"></div>
        <div class="DxSlider__rail-overlay DxSlider__rail-overlay--mid"></div>
        <div class="DxSlider__rail-overlay DxSlider__rail-overlay--post"></div>
          ${handleHtml('DxSlider__thumb--first')}
          ${secondThumb()}
      </div>
    `;
    return html;
...
const dxSliders = document.querySelectorAll('.DxSlider');
if (dxSliders.length) {
  Array.prototype.forEach.call(dxSliders, el => new DxSlider(el));
}
